...
Full plan (Fireworks SFT primary, todos): [extraction-ft-lean-plan.md](./extraction-ft-lean-plan.md). Baseline eval commands: [extraction-ft-lean-baseline.md](./extraction-ft-lean-baseline.md). Append-only log: [extraction-ft-lean-iteration-log.md](./extraction-ft-lean-iteration-log.md). Combined eval: `pnpm ops:eval-extraction-compare`. Golden A/B + speed/reliability notes: [extraction-ft-golden-ab-recommendations.md](./extraction-ft-golden-ab-recommendations.md).

---

## Availability, capacity, and alternative hosting

On-demand Fireworks deployments can hit **`RESOURCE_EXHAUSTED`** / **no available capacity**, **multi-region** placement constraints (e.g. cannot change accelerator while **GLOBAL** sharding is on), and **quota** lookups that fail when pinning a single region without matching entitlement. Offline eval may look great while **scheduling** is flaky — plan for **ops**, not only model quality.

### Highest-leverage pattern: primary + fallback

- **Primary:** Fine-tuned extraction on **Fireworks** (or another dedicated GPU host) when it is up.
- **Fallback:** Production **`vertex:…`** Gemini (e.g. `gemini-3-flash-preview`) when the primary returns **503**, **RESOURCE_EXHAUSTED**, or **timeouts** after bounded retries — so **ingestion completes** even if the fast path is unavailable.

Implementing that is an **ingestion routing** change (env + code), not a Fireworks CLI tweak. Until it exists, treat **Vertex** as the **dependable** default for live extraction; see [extraction-ft-lean-plan.md](./extraction-ft-lean-plan.md) § *Production extraction model — Vertex*.

### If Fireworks is not dependable enough as sole host

| Option | Idea | Trade-off |
|--------|------|-----------|
| **Vertex AI (GCP)** | Serve the **same** merged HF weights (or imported model) on a **Vertex endpoint** or **custom container** in a region you quota — same cloud and IAM as prod Gemini. | Engineering work to package/serve; predictable once live. |
| **GKE / Compute Engine + GPU** | **vLLM**, **TGI**, or similar on **reserved** GPU nodes you control. | Full ops ownership; best control over availability vs cost. |
| **Another managed GPU API** | e.g. **Together**, **Baseten**, **Modal**, **Replicate**, **Anyscale**-class hosts — different capacity pool than Fireworks; often **dedicated** or **provisioned** tiers. | New vendor relationship; still subject to their capacity story. |
| **Hyperscaler ML endpoints** | **SageMaker**, **Azure ML**, **Bedrock** (if model fits import/catalog). | Fits teams already on those clouds. |

No option removes **all** capacity risk; **multi-route** (FT when healthy, Vertex otherwise) usually improves **user-visible reliability** more than swapping one SaaS GPU vendor for another without a fallback.