...
| **You** | **Fireworks SFT (preferred):** Use **`--base-model accounts/adam-boon1984-17nryg/models/sophia-extract-m7b-ft`** for SFT from the merged upload (**verified Tunable** 2026-04-16 — see [extraction-ft-lean-plan.md](./extraction-ft-lean-plan.md)); re-check `firectl model get` if the model is recreated. Later iterations: **`--warm-start-from`** prior SFT output. Pick `--output-model` slug; approve **Fireworks training spend**; `pnpm ops:fireworks-submit-sft -- --dry-run` then live ([extraction-fireworks-deploy.md](./extraction-fireworks-deploy.md) addendum). Optionally set **`FIREWORKS_ACCOUNT_ID`** if `EXTRACTION_MODEL` is unset. |
