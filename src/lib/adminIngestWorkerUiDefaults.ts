...
export const ADMIN_INGEST_WORKER_UI_TOOLTIPS = {
	...
	validationTailOnly:
		'Passes `--force-stage validating` to `scripts/ingest.ts`: skips extract / relate / group / embed only if this **same** `INGEST_ORCHESTRATION_RUN_ID` already has checkpoints through embedding (Neon staging + partial). `INGEST_FORCE_REINGEST` is stripped in this mode. On a **brand-new** durable job item the child run id is new, so the **first** start has no such checkpoints — leave this off until that URL has finished through embedding once (or use `scripts/replay-ingest.ts` / admin resume on an existing run). Remediation re-embeds only edited claims. If Surreal `ingestion_log.stage_completed` is already past embedding (e.g. `remediating` after remediation, or deploy mid-store), ingest **does not rewind** to validation — it resumes after the later of the force floor and that checkpoint (store-only when remediation is done). If the tail checkpoint is missing, set worker env `INGEST_FORCE_STAGE_MISSING_CHECKPOINT=full` (re-run from extraction) or `=resume` (best partial without embedding-complete gate) instead of failing. Optional: set worker env `INGEST_SKIP_STORE_WHEN_NO_GRAPH_CHANGES=1` with `--validate` to skip Surreal Stage 6 when post-validation graph mutations are absent and a `source` row already exists (ingestion_log stays at validating/remediating until a full store).',
