...
import { cancelBatchItem } from '$lib/server/stoaIngestionBatch';
...
export const POST: RequestHandler = async ({ locals, params, request }) => {
	...
	try {
		const run = await cancelBatchItem(runId, queueRecordId);
		if (!run) return json({ error: 'Batch run not found' }, { status: 404 });
		return json({ run });
	} catch (error) {
		const message = error instanceof Error ? error.message : 'Failed to cancel batch item';
