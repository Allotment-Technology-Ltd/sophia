...
export const GET: RequestHandler = async ({ locals, url }) => {
  ...
  for (const provider of REASONING_PROVIDER_ORDER) {
    const ids = new Set<string>(DEFAULT_MODEL_CATALOG[provider] ?? []);
    for (const model of candidateModels) {
      if (model.provider === provider) ids.add(model.id);
    }
    ...
    for (const modelId of ids) {
      try {
        ...
        if (result.data.allowed) {
          allowedByProvider[provider]!.push(modelId);
        }
      } catch (err) {
        if (process.env.NODE_ENV !== 'test') {
          console.warn(
            '[restormel] Failed to evaluate allowed models; returning local fallback:',
            err instanceof Error ? err.message : String(err)
          );
        }
        return json({
