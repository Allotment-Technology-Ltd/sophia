...
<main class="expand-page sophia-stack-comfortable">
  <header class="expand-hero">
    <p class="font-mono text-xs uppercase tracking-[0.12em] text-sophia-dark-dim">Admin</p>
    <h1 class="mt-2 font-serif text-3xl text-sophia-dark-text sm:text-[2.1rem]">Quarantine review</h1>
    <p class="mt-2 max-w-3xl text-sm leading-6 text-sophia-dark-muted">
      Claims with low validation scores, non-validated verification, or review flags. Select rows and run
      agent remediation (same passage-bounded repair contract as ingest Stage 5b).
    </p>
  </header>

  <section class="sophia-stack-default" aria-label="Filters">
    <div class="flex flex-wrap items-end gap-4">
      <label class="flex flex-col gap-1 text-sm text-sophia-dark-muted">
        Max validation score (exclude above)
        <input
          type="number"
          class="min-h-[44px] rounded border border-sophia-dark-border bg-sophia-dark-surface px-3 py-2 font-mono text-sm text-sophia-dark-text"
          min="0"
          max="100"
          bind:value={maxScore}
        />
      ...
      <button
        type="button"
        class="min-h-[44px] rounded border border-sophia-dark-border bg-sophia-dark-surface px-4 py-2 font-mono text-xs uppercase tracking-[0.1em] text-sophia-dark-text hover:border-sophia-dark-dim"
        onclick={() => loadQueue()}
        disabled={loading}
      >
        Refresh
      ...
      <button
        type="button"
        class="min-h-[44px] rounded border border-sophia-dark-sage bg-sophia-dark-sage px-4 py-2 font-mono text-xs uppercase tracking-[0.1em] text-sophia-dark-bg hover:opacity-90 disabled:opacity-50"
    ...
    {#if actionMsg}
      <p class="text-sm text-sophia-dark-muted">{actionMsg}</p>
    {/if}
    {#if loadError}
      <p class="text-sm text-sophia-dark-coral">{loadError}</p>
    {/if}
  </section>
  ...
  <section aria-label="Queue table">
    ...
    {:else}
      <div class="overflow-x-auto rounded border border-sophia-dark-border">
        <table class="w-full min-w-[720px] border-collapse text-left text-sm">
          <thead>
            <tr class="border-b border-sophia-dark-border bg-sophia-dark-surface font-mono text-xs uppercase tracking-[0.08em] text-sophia-dark-dim">
          ...
          <tbody>
            {#each rows as row (row.id)}
              <tr class="border-b border-sophia-dark-border/60">
                ...
                <td class="p-3 align-top text-xs text-sophia-dark-muted">
                  {#if row.source_url}
                    <a href={row.source_url} class="text-sophia-dark-sage underline" target="_blank" rel="noreferrer"
                      >{row.source_title ?? row.source_url}</a
                    >
                  {:else}
                    {row.source_title ?? '—'}
                  {/if}
