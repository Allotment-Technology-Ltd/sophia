...
async function main() {
	...
	try {
		...
		if (resumeFromStage && ['extraction', 'relations', 'grouping', 'embedding', 'validation'].includes(resumeFromStage)) {
			console.log('┌──────────────────────────────────────────────────────────┐');
			console.log('│ STAGE 1: CLAIM EXTRACTION (SKIPPED - RESUMING)         │');
			console.log('└──────────────────────────────────────────────────────────┘');
			if (!partial.claims) {
				throw new Error('Cannot resume: claims missing from partial results');
			}
			allClaims = partial.claims;
			console.log(`  [RESUME] Loaded ${allClaims.length} claims from partial results`);
			console.log('');
		} else {
			console.log('┌──────────────────────────────────────────────────────────┐');
		...
		if (resumeFromStage && ['relations', 'grouping', 'embedding', 'validation'].includes(resumeFromStage)) {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
			console.log('│ STAGE 2: RELATION EXTRACTION (SKIPPED - RESUMING)      │');
			console.log('└──────────────────────────────────────────────────────────┘');
			if (!partial.relations) {
				throw new Error('Cannot resume: relations missing from partial results');
			}
			relations = partial.relations;
			console.log(`  [RESUME] Loaded ${relations.length} relations from partial results`);
		} else {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
		...
		if (resumeFromStage && ['grouping', 'embedding', 'validation'].includes(resumeFromStage)) {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
			console.log('│ STAGE 3: ARGUMENT GROUPING (SKIPPED - RESUMING)        │');
			console.log('└──────────────────────────────────────────────────────────┘');
			if (!partial.arguments) {
				throw new Error('Cannot resume: arguments missing from partial results');
			}
			arguments_ = partial.arguments;
			console.log(`  [RESUME] Loaded ${arguments_.length} arguments from partial results`);
		} else {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
		...
		if (resumeFromStage && ['embedding', 'validation'].includes(resumeFromStage)) {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
			console.log('│ STAGE 4: EMBEDDING (SKIPPED - RESUMING)                │');
			console.log('└──────────────────────────────────────────────────────────┘');
			if (!partial.embeddings) {
				throw new Error('Cannot resume: embeddings missing from partial results');
			}
			allEmbeddings = partial.embeddings;
			console.log(`  [RESUME] Loaded ${allEmbeddings.length} embeddings from partial results`);
		} else {
			console.log('\n┌──────────────────────────────────────────────────────────┐');
		...
		if (shouldValidate) {
			if (resumeFromStage === 'validation') {
				console.log('\n┌──────────────────────────────────────────────────────────┐');
				console.log('│ STAGE 5: VALIDATION (SKIPPED - RESUMING)               │');
				console.log('└──────────────────────────────────────────────────────────┘');
				validationResult = partial.validation || null;
				console.log(`  [RESUME] Loaded validation results from partial results`);
			} else {
				console.log('\n┌──────────────────────────────────────────────────────────┐');
