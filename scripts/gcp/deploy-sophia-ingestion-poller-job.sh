...
# INGEST_EMBEDDING_IGNORE_LEGACY_CORPUS_DIM: allow Voyage 1024 writes while legacy 768 claim rows remain (remove after full re-embed).
ENV_BLOCK="^|^ADMIN_INGEST_RUN_REAL=1|INGEST_QUEUE_ENABLED=0|SOPHIA_DATA_BACKEND=neon|USE_NEON_AUTH=1|NEON_AUTH_BASE_URL=${NEON_AUTH_BASE_URL}|PUBLIC_NEON_AUTH_URL=${NEON_AUTH_BASE_URL}|GCP_PROJECT_ID=${PROJECT_ID}|GCP_LOCATION=${REGION}|GOOGLE_VERTEX_LOCATION=us-central1|ALLOWED_EMAILS=${ALLOWED_EMAILS}|OWNER_EMAILS=${OWNER_EMAILS}|PADDLE_RUNTIME=production|BYOK_DISABLE_CLOUD_KMS_ENCRYPT=1|INGEST_EMBEDDING_IGNORE_LEGACY_CORPUS_DIM=1|RESTORMEL_GATEWAY_KEY=${RESTORMEL_GATEWAY_KEY}|RESTORMEL_PROJECT_ID=${RESTORMEL_PROJECT_ID}|RESTORMEL_ENVIRONMENT_ID=${RESTORMEL_ENVIRONMENT_ID}|RESTORMEL_KEYS_BASE=${RESTORMEL_KEYS_BASE}|RESTORMEL_BASE_URL=${RESTORMEL_BASE_URL}|RESTORMEL_ANALYSE_ROUTE_ID=${RESTORMEL_ANALYSE_ROUTE_ID}|RESTORMEL_VERIFY_ROUTE_ID=${RESTORMEL_VERIFY_ROUTE_ID}|RESTORMEL_PROJECT_MODEL_REGISTRY_BINDINGS=1"
