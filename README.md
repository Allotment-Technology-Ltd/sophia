...
- GCP project with Vertex AI enabled (for ingestion pipeline + query-time model calls)
- Anthropic API key (optional; only needed for manual ingestion rollback via `--ingest-provider anthropic`)
